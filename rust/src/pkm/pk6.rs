...
use crate::pkm::types::{Stats, ContestStats, FlagSet, Gender};
...
impl Pkm<232, 260> for PK6 {
	...
	fn from_bytes(bytes: &[u8]) -> Result<Self, String> {
		...
		if size < 232 {
			return Err(format!(
				"byte array is too short for PK6 format (received {size}, expected >= 232)"
			));
		}
		let mon = PK6 {
			...
			nature: bytes[28],
			forme_num: util::int_from_buffer_bits_le::<u8>(bytes, 29, 3, 5).map_err(|e| format!("read field 'forme_num': {e}"))?,
			gender: util::int_from_buffer_bits_le::<u8>(bytes, 29, 1, 2).map_err(|e| format!("read field 'gender': {e}"))?.into(),
			evs: Stats::from_bytes(to_sized_array(&bytes[30..36])),
			contest: ContestStats::from_bytes(to_sized_array(&bytes[36..42])),
			pokerus_byte: bytes[43],
			super_training_flags: u32::from_le_bytes(to_sized_array(&bytes[44..48])),
			ribbon_bytes: FlagSet::from_bytes(to_sized_array(&bytes[48..54])),
			...
			moves: [u16::from_le_bytes(to_sized_array(&bytes[90..92])), u16::from_le_bytes(to_sized_array(&bytes[92..94])), u16::from_le_bytes(to_sized_array(&bytes[94..96])), u16::from_le_bytes(to_sized_array(&bytes[96..98]))],
			move_pp: [bytes[98], bytes[99], bytes[100], bytes[101]],
			move_pp_ups: [bytes[102], bytes[103], bytes[104], bytes[105]],
			relearn_moves: [u16::from_le_bytes(to_sized_array(&bytes[106..108])), u16::from_le_bytes(to_sized_array(&bytes[108..110])), u16::from_le_bytes(to_sized_array(&bytes[110..112])), u16::from_le_bytes(to_sized_array(&bytes[112..114]))],
