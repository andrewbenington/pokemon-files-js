...
use crate::pkm::types::{Gender, StatsPreSplit};
use crate::pkm::util;
...
impl Pkm<32, 73> for PK2 {
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self, String> {
        ...
        let mon = PK2 {
            ...
            move_pp: [
                util::int_from_buffer_bits_be::<u8>(bytes, 23, 0, 6)
                    .map_err(|e| format!("read field 'move_pp[0]': {e}"))?,
                util::int_from_buffer_bits_be::<u8>(bytes, 24, 6, 6)
                    .map_err(|e| format!("read field 'move_pp[1]': {e}"))?,
                util::int_from_buffer_bits_be::<u8>(bytes, 25, 12, 6)
                    .map_err(|e| format!("read field 'move_pp[2]': {e}"))?,
                util::int_from_buffer_bits_be::<u8>(bytes, 26, 18, 6)
                    .map_err(|e| format!("read field 'move_pp[3]': {e}"))?,
            ],
            move_pp_ups: [
                util::int_from_buffer_bits_be::<u8>(bytes, 23, 6, 2)
                    .map_err(|e| format!("read field 'move_pp_ups[0]': {e}"))?,
                util::int_from_buffer_bits_be::<u8>(bytes, 24, 8, 2)
                    .map_err(|e| format!("read field 'move_pp_ups[1]': {e}"))?,
                util::int_from_buffer_bits_be::<u8>(bytes, 25, 10, 2)
                    .map_err(|e| format!("read field 'move_pp_ups[2]': {e}"))?,
                util::int_from_buffer_bits_be::<u8>(bytes, 26, 12, 2)
                    .map_err(|e| format!("read field 'move_pp_ups[3]': {e}"))?,
            ],
            trainer_friendship: bytes[27],
            pokerus_byte: bytes[28],
            met_time_of_day: util::int_from_buffer_bits_be::<u8>(bytes, 29, 6, 2)
                .map_err(|e| format!("read field 'met_time_of_day': {e}"))?,
            met_level: util::int_from_buffer_bits_be::<u8>(bytes, 29, 0, 6)
                .map_err(|e| format!("read field 'met_level': {e}"))?,
            met_location_index: util::int_from_buffer_bits_be::<u8>(bytes, 30, 0, 7)
                .map_err(|e| format!("read field 'met_location_index': {e}"))?,
            level: bytes[31],
            status_condition: if bytes.len() > 70 { bytes[32] } else { 0 },
            current_hp: if bytes.len() > 70 { bytes[34] } else { 0 },
            trainer_name: if bytes.len() > 70 {
                GbString::from_bytes(to_sized_array::<[u8; 8], u8>(&bytes[48..56]))
            } else {
                "TRAINER".into()
            },
            nickname: if bytes.len() > 70 {
                GbString::from_bytes(to_sized_array::<[u8; 11], u8>(&bytes[59..70]))
            } else {
                "NICKNAME".into()
            },
