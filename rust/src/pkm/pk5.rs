...
use crate::pkm::util;
use crate::pkm::strings::Gen5String;
...
impl Pkm<136, 236> for PK5 {
	...
	fn from_bytes(bytes: &[u8]) -> Result<Self, String> {
		...
		if size < 136 {
			return Err(format!(
				"byte array is too short for PK5 format (received {size}, expected >= 136)"
			));
		}
		let mon = PK5 {
			...
			moves: [u16::from_le_bytes(to_sized_array(&bytes[40..42])), u16::from_le_bytes(to_sized_array(&bytes[42..44])), u16::from_le_bytes(to_sized_array(&bytes[44..46])), u16::from_le_bytes(to_sized_array(&bytes[46..48]))],
			...
			gender: util::int_from_buffer_bits_le::<u8>(bytes, 64, 1, 2).map_err(|e| format!("read field 'gender': {e}"))?.into(),
			forme_num: util::int_from_buffer_bits_le::<u8>(bytes, 64, 3, 5).map_err(|e| format!("read field 'forme_num': {e}"))?,
			nature: bytes[65],
			is_ns_pokemon: util::get_flag(bytes, 66, 1),
			ribbon_bytes: FlagSet::from_bytes(to_sized_array(&bytes[76..80])),
			game_of_origin: bytes[95],
			egg_location_index: u16::from_le_bytes(to_sized_array(&bytes[126..128])),
			met_location_index: u16::from_le_bytes(to_sized_array(&bytes[128..130])),
			...
			nickname: Gen5String::from_bytes(to_sized_array::<[u8; 24], u8>(&bytes[72..96])),
			trainer_name: Gen5String::from_bytes(to_sized_array::<[u8; 16], u8>(&bytes[104..120])),
	...
	fn write_bytes(&self, bytes: &mut [u8; 136]) {
				...
				bytes[64] = self.gender.into();
				bytes[64] = self.forme_num;
				bytes[65] = self.nature;
