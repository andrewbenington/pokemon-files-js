...
impl Pkm<80, 100> for PK3 {
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self, String> {
        ...
        let mon = PK3 {
            ...
            move_pp_ups: [
                util::int_from_buffer_bits_le::<u8>(bytes, 40, 0, 2)
            ...
            moves: [
                ...
                u16::from_le_bytes(to_sized_array(&bytes[48..50])),
                u16::from_le_bytes(to_sized_array(&bytes[50..52])),
            ],
            ...
            met_level: util::int_from_buffer_bits_le::<u8>(bytes, 70, 0, 7)
                .map_err(|e| format!("read field 'met_level': {e}"))?,
            game_of_origin: util::int_from_buffer_bits_le::<u8>(bytes, 70, 7, 4)
                .map_err(|e| format!("read field 'game_of_origin': {e}"))?,
            ball: util::int_from_buffer_bits_le::<u8>(bytes, 70, 11, 4)
                .map_err(|e| format!("read field 'ball': {e}"))?,
