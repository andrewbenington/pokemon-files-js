...
impl Pkm<33, 66> for PK1 {
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self, String> {
        ...
        let mon = PK1 {
            ...
            move_pp: [
                util::int_from_buffer_bits_be::<u8>(bytes, 29, 0, 6)
                    .map_err(|e| format!("read field 'move_pp[0]': {e}"))?,
                util::int_from_buffer_bits_be::<u8>(bytes, 30, 6, 6)
                    .map_err(|e| format!("read field 'move_pp[1]': {e}"))?,
                util::int_from_buffer_bits_be::<u8>(bytes, 31, 12, 6)
                    .map_err(|e| format!("read field 'move_pp[2]': {e}"))?,
                util::int_from_buffer_bits_be::<u8>(bytes, 32, 18, 6)
                    .map_err(|e| format!("read field 'move_pp[3]': {e}"))?,
            ],
            move_pp_ups: [
                util::int_from_buffer_bits_be::<u8>(bytes, 29, 6, 2)
                    .map_err(|e| format!("read field 'move_pp_ups[0]': {e}"))?,
                util::int_from_buffer_bits_be::<u8>(bytes, 30, 8, 2)
                    .map_err(|e| format!("read field 'move_pp_ups[1]': {e}"))?,
                util::int_from_buffer_bits_be::<u8>(bytes, 31, 10, 2)
                    .map_err(|e| format!("read field 'move_pp_ups[2]': {e}"))?,
                util::int_from_buffer_bits_be::<u8>(bytes, 32, 12, 2)
                    .map_err(|e| format!("read field 'move_pp_ups[3]': {e}"))?,
            ],
            trainer_name: if bytes.len() > 66 {
                GbString::from_bytes(to_sized_array::<[u8; 8], u8>(&bytes[44..52]))
            } else {
                "TRAINER".into()
            },
            nickname: if bytes.len() > 66 {
                GbString::from_bytes(to_sized_array::<[u8; 11], u8>(&bytes[55..66]))
            } else {
                "NICKNAME".into()
            },
        };
