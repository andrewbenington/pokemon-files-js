...
import { Languages, NatureToString, ItemGen3ToString, ItemGen3FromString, Ball, Gen3ContestRibbons, Gen3StandardRibbons } from 'pokemon-resources'
...
export class XDPKM {
  static getName() {return 'XDPKM'}
  format: 'XDPKM' = 'XDPKM'
  static getBoxSize() {return 196 }
  dexNum: number
...
dataView.getUint8(0x86),
      ...
      const other = arg
        ...
        this.movePP = adjustMovePPBetweenFormats(this, other).filter((_, i) => other.moves[i] <= XDPKM.maxValidMove())
...
byteLogic.setFlagIndexes(dataView, 0x7c, 15, this.ribbons.map(ribbon => Gen3StandardRibbons.indexOf(ribbon)).filter(index => index > -1 && index < 12))
    return buffer;
  }
