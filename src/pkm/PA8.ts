...
import { Languages, ItemToString, ItemFromString, AbilityToString, NatureToString, Ball, ModernRibbons } from 'pokemon-resources'
...
export class PA8 {
  static getName() {return 'PA8'}
  format: 'PA8' = 'PA8'
  static getBoxSize() {return 360 }
...
dataView.getUint16(0x90, true),
        ...
        this.ribbons = byteLogic.getFlagIndexes(dataView, 0x34, 0, 64).map(index => ModernRibbons[index]).concat(byteLogic.getFlagIndexes(dataView, 0x40, 0, 47).map(index => ModernRibbons[index + 64]))
      ...
      const other = arg
        ...
        this.markings = types.markingsSixShapesWithColorFromOther(other.markings) ?? { circle: false, triangle: false, square: false, heart: false, star: false, diamond: false }
        ...
        this.movePP = adjustMovePPBetweenFormats(this, other).filter((_, i) => other.moves[i] <= PA8.maxValidMove())
        this.nickname = other.nickname
        this.movePPUps = other.movePPUps.filter((_, i) => other.moves[i] <= PA8.maxValidMove())
        this.relearnMoves = other.relearnMoves?.filter((_, i) => other.moves[i] <= PA8.maxValidMove()) ?? [0, 0, 0, 0]
...
dataView.setUint16(0x8a + i * 2, this.relearnMoves[i], true)
        ...
        byteLogic.setFlagIndexes(dataView, 0x34, 0, this.ribbons.map(ribbon => ModernRibbons.indexOf(ribbon)).filter(index => index > -1 && index < 64))
byteLogic.setFlagIndexes(dataView, 0x40, 0, this.ribbons.map(ribbon => ModernRibbons.indexOf(ribbon) - 64).filter(index => index > -1 && index < 47))
  ...
  isShiny() {
    return (
      (this.trainerID ^
        ...
        ((this.personalityValue >> 16) & 0xffff)) <16
    )
  }
