...
types.readGeolocationFromBytes(dataView, 0x9c),
        ...
        this.ribbons = byteLogic.getFlagIndexes(dataView, 0x30, 0, 46).map(index => ModernRibbons[index])
      ...
      const other = arg
        ...
        this.markings = types.markingsSixShapesNoColorFromOther(other.markings) ?? { circle: false, triangle: false, square: false, heart: false, star: false, diamond: false }
        ...
        this.movePP = adjustMovePPBetweenFormats(this, other).filter((_, i) => other.moves[i] <= PK6.maxValidMove())
        this.movePPUps = other.movePPUps.filter((_, i) => other.moves[i] <= PK6.maxValidMove())
        this.relearnMoves = other.relearnMoves?.filter((_, i) => other.moves[i] <= PK6.maxValidMove()) ?? [0, 0, 0, 0]
...
types.writeGeolocationToBytes(dataView, 0x94 + 2 * i, this.geolocations[i])
        ...
        byteLogic.setFlagIndexes(dataView, 0x30, 0, this.ribbons.map(ribbon => ModernRibbons.indexOf(ribbon)).filter(index => index > -1 && index < 46))
