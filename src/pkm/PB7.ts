...
dataView.getUint16(0x70, true),
      ...
      const other = arg
        ...
        this.movePP = adjustMovePPBetweenFormats(this, other).filter((_, i) => other.moves[i] <= PB7.maxValidMove())
        this.movePPUps = other.movePPUps.filter((_, i) => other.moves[i] <= PB7.maxValidMove())
        this.relearnMoves = other.relearnMoves?.filter((_, i) => other.moves[i] <= PB7.maxValidMove()) ?? [0, 0, 0, 0]
        this.ivs = other.ivs ?? {
		hp: 0,
		atk: 0,
		def: 0,
