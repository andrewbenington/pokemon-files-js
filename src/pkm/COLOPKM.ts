...
import { Languages, NatureToString, Ball, ItemGen3ToString, ItemGen3FromString, Gen3ContestRibbons, Gen3StandardRibbons } from 'pokemon-resources'
...
export class COLOPKM {
  static getName() {return 'COLOPKM'}
  format: 'COLOPKM' = 'COLOPKM'
  static getBoxSize() {return 312 }
  dexNum: number
...
dataView.getUint8(0x7e),
      ...
      const other = arg
        ...
        this.movePP = adjustMovePPBetweenFormats(this, other).filter((_, i) => other.moves[i] <= COLOPKM.maxValidMove())
        ...
        this.markings = types.markingsFourShapesFromOther(other.markings) ?? { circle: false, triangle: false, square: false, heart: false }
...
byteLogic.setByteIndexes(dataView, 0xbd, this.ribbons.map(ribbon => Gen3StandardRibbons.indexOf(ribbon) - 0).filter(index => index > -1 && index < 11))
    return buffer;
  }
  ...
  isShiny() {
    return (
      (this.trainerID ^
        ...
        ((this.personalityValue >> 16) & 0xffff)) <8
    )
  }
