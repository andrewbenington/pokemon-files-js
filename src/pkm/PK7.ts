...
export class PK7 {
  static getName() {return 'PK7'}
  format: 'PK7' = 'PK7'
  static getBoxSize() {return 232 }
...
types.readGeolocationFromBytes(dataView, 0x9c),
        ...
        this.ribbons = byteLogic.getFlagIndexes(dataView, 0x30, 0, 50).map(index => ModernRibbons[index])
      ...
      const other = arg
        ...
        this.movePP = adjustMovePPBetweenFormats(this, other).filter((_, i) => other.moves[i] <= PK7.maxValidMove())
        this.movePPUps = other.movePPUps.filter((_, i) => other.moves[i] <= PK7.maxValidMove())
        this.relearnMoves = other.relearnMoves?.filter((_, i) => other.moves[i] <= PK7.maxValidMove()) ?? [0, 0, 0, 0]
...
types.writeGeolocationToBytes(dataView, 0x94 + 2 * i, this.geolocations[i])
        ...
        byteLogic.setFlagIndexes(dataView, 0x30, 0, this.ribbons.map(ribbon => ModernRibbons.indexOf(ribbon)).filter(index => index > -1 && index < 50))
